...
void Connection::connectContext(const QObject *querytool)
{
...
void Connection::run()
{
	...
	if(m_db.open()) {
		...
		connect(this, SIGNAL(needNewContext(const QObject*)), ctxman, SLOT(createContext(const QObject*)), Qt::BlockingQueuedConnection);
